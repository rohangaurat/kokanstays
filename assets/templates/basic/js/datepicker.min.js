...
; (function ($) {
    ...
    var Defaults = {
         ...
         fnDataEvent: null,
         inputOneName: 'check_in',
         inputTwoName: 'check_out'
    };
    ...
    $.fn.tDatePicker = function (pr_el, options) {
         ...
         update_options.formatDate = update_options.formatDate || Defaults.formatDate; update_options.startDate = update_options.startDate || toDay;
         ...
         function getDateUTC(pr_in, pr_out) {
              var Array_In_Out = ['check-in', 'check-out']; Array_In_Out.forEach(function (e) {
                   ...
                   var getDay = pr_in;
                   var Input = settings.inputOneName;
                   if (pr_in !== null) { label_title = ''; }
                   if (e === 'check-out') {
                        label_title = settings.titleCheckOut; getDay = pr_out;
         ...
         function getTableCalendar(pr_el, pr_date_utc) {
              if ($(pr_el).find('.t-datepicker-day').length !== 0) {
                   ...
                   $(pr_el).find('.t-dates').parent().append(
                        '<div class="t-datepicker-day' + checkNumCalendar + '"> ' + convertArrayToString(dataTheme) +

                        '<div style="margin-top:10px; text-align:center;"><button class="ti-close-btn" style="border-radius:5px; padding:8px 10px; background-color:#000; color:#fff;" type="button">Close</button></div>' +
                        '</div>'
                   );

                   $(pr_el).find('.ti-close-btn').on('click', function () {
                        $('.t-datepicker-day').remove();
                        $('.t-arrow-top').remove();
                        $('body').removeClass('ti-disable-scroll');
                        $('body').find('.banner-section').removeClass('banner-zindex');
                   });
