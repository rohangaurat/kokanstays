    ...
    public function handle()
    {
        ...
        $routesPermissions = collect(Route::getRoutes())
            ->filter(function ($route) use ($excludedActions) {
                return str_starts_with($route->getName(), 'owner.') && !in_array(last(array_reverse(explode('@', class_basename($route->getAction('controller'))))), $excludedActions) && !in_array(class_basename($route->getAction('controller')), $excludedActions);
            })
            ->map(function ($route) {
                $controller = explode('@', class_basename($route->getActionName()));
                return [
                    'code' => $route->getName(),
                    'name' => ucwords(str_replace('.', ' ', str_replace('owner.', '', $route->getName()))),
                    'group' => @$controller[0],
                ];
            });
        $newRoutes = [];
