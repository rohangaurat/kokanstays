    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if (now() >= $paymentDeadLine) {
            if (!authOwner()->parent_id) {
                $message = authOwner()->deposits()->where('pay_for_month', '>', 0)->count() > 0 ? 'Your payment deadline has been exceed, you have to pay first.' : 'You have to pay monthly bill first.';
