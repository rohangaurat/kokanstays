                                ...
                                @forelse($methods as $method)
                                    <tr>
                                        <td>
                                            <div class="user">
                                                <div class="thumb"><img
                                                        src="{{ getImage(getFilePath('withdrawMethod') . '/' . $method->image) }}"
                                                        alt="@lang('image')"></div>
                                        ...
                                        <td class="fw-bold">{{ showAmount($method->fixed_charge) }}
                                            {{ 0 < $method->percent_charge ? ' + ' . showAmount($method->percent_charge, currencyFormat: false) . ' %' : '' }}
                                        </td>
                                        <td class="fw-bold">
                                            {{ showAmount($method->min_limit) }} @lang('to')
...
@push('breadcrumb-plugins')
    ...
    <a class="btn btn-outline--primary" href="{{ route('admin.withdraw.method.create') }}">
