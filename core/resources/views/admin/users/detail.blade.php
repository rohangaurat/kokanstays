        ...
        <div class="col-12">
            <div class="row gy-4">
                <div class="col-xxl-3 col-sm-6">
                    <x-widget bg="info" icon="las la-wallet" style="3" title="Total Bookings"
                        value="{{ $widget['total_bookings'] }}" />
                </div>
                <div class="col-xxl-3 col-sm-6">
                    <x-widget bg="success" icon="las la-hotel" style="3" title="Running Bookings"
                        value="{{ $widget['running_bookings'] }}" />
                </div>
                <div class="col-xxl-3 col-sm-6">
                    <x-widget bg="warning" icon="las la-hotel" style="3" title="Booking Request"
                        value="{{ $widget['booking_requests'] }}" />
                </div>
                <div class="col-xxl-3 col-sm-6">
                    <x-widget bg="3" icon="las la-wallet" style="3" title="Total Payment"
                        value="{{ showAmount($widget['total_payment']) }}" />
                </div>
            </div>
            ...
            <div class="card mt-30">
                <div class="card-header d-flex justify-content-between align-content-center flex-wrap gap-2">
                    <h5 class="card-title mb-0">
                        @lang('Information of') {{ __($user->fullname) }}
                        @if ($user->deleted_at)
                            <span class="badge badge--danger">@lang('Deleted')</span>
                        @endif
                    </h5>
                    @if ($user->deleted_at)
                        <button class="btn btn-sm btn-outline--dark confirmationBtn"
                            data-action="{{ route('admin.users.restore', $user->id) }}" data-question="@lang('Are you sure that you want to restore this user\'s account?')">
                            <i class="las la-trash-restore-alt"></i> @lang('Restore')
                        </button>
                    @else
                        <button class="btn btn-sm btn-outline--danger confirmationBtn"
                            data-action="{{ route('admin.users.delete', $user->id) }}" data-question="@lang('Are you sure that you want to delete this user\'s account?')">
                            <i class="las la-trash-alt"></i> @lang('Delete Account')
                        </button>
                    @endif
                </div>
                <div class="card-body">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        {{ $user->status == Status::USER_ACTIVE ? __('Ban User') : __('Unban User') }}
                    </h5>
