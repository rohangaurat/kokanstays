            ...
            <div class="row  mb-4 justify-content-center">
                <div class="col-sm-7">
                    <div class="row gy-4 justify-content-center">
                        <div class="col-sm-6">
                            <x-widget style="6" icon="fas fa-list" :title="$viaText . ' should be sent'" value="{{ @$sessionData['total_owner'] }}" bg="primary" :viewMoreIcon=false />
                        </div>
                        <div class="col-sm-6">
                            <x-widget style="6" icon="fa-solid fa-envelope-circle-check" :title="$viaText . ' has been sent'" value="{{ @$sessionData['total_sent'] }}" bg="success" :viewMoreIcon=false />
                        </div>
                        <div class="col-sm-6">
                            <x-widget style="6" icon="fa-solid fa-paper-plane" :title="$viaText . ' has yet to be sent'" :viewMoreIcon=false value="{{ @$sessionData['total_owner'] - @$sessionData['total_sent'] }}" bg="warning" />
                        </div>
                        <div class="col-sm-6">
                            <x-widget style="6" icon="fas fa-envelope" :title="$viaText . ' per batch'" value="{{ @$sessionData['batch'] }}" bg="primary" :viewMoreIcon=false />
                        </div>
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body p-5 text-center">
                                    <div class="coaling-loader flex-column d-flex justify-content-center">
                                        ...
                                        <p class="text--primary">
                                            @lang(' ' . @$sessionData['total_sent'] . ' out of ' . @$sessionData['total_owner'] . ' ' . $viaName . ' were successfully transmitted')
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="being_sent_to" required data-minimum-results-for-search="1">
                                        @foreach ($notifyToOwner as $key => $toOwner)
                                            <option value="{{ $key }}" @selected(old('being_sent_to', @$sessionData['being_sent_to']) == $key)>{{ __($toOwner) }}</option>
                                        @endforeach
                                    </select>
                                    <small class="text--info d-none ownerCountText"> <i class="las la-info-circle"></i> <strong class="ownerCount">0</strong> @lang('active owners found to send the notification')</small>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-4 start-from-col">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" name="start" value="{{ old('start', @$sessionData['start']) }}" type="number" placeholder="@lang('Start form owner id. e.g. 1')" required>
                                        </div>
                                    </div>
                                    <div class="col-md-4 per-batch-col">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input class="form-control" name="batch" value="{{ old('batch', @$sessionData['batch']) }}" type="number" placeholder="@lang('How many owner')" required>
    ...
    <script>
        ...
        (function($) {
            ...
            $('select[name=being_sent_to]').on('change', function(e) {
                ...
                getOwnerCount(methodName);
                methodName = methodName.toUpperCase();

                if (methodName == 'SELECTEDOWNERS') {
                    $('.input-append').html(`
                    <div class="form-group" id="owner_list_wrapper">
                        <label class="required">@lang('Select Vendor')</label>
                        <select name="owner[]"  class="form-control" id="owner_list" required multiple >
                ...
                if (methodName == 'TOPDEPOSITEDOWNERS') {
                    $('.input-append').html(`
                    <div class="form-group">
                        <label class="required">@lang('Number Of Top Deposited Owner')</label>
                        <input class="form-control" type="number" name="number_of_top_deposited_owner" >
                    </div>
                    `);
                    return;
                }

                if (methodName == 'NOTLOGINOWNERS') {
                    $('.input-append').html(`
                    <div class="form-group">
            ...
            function fetchOwnerList() {
                ...
                $('.row #owner_list').select2({
                    ajax: {
                        url: "{{ route('admin.owners.list') }}",
                        type: "get",
                        dataType: 'json',
                        delay: 1000,
                    ...
                    dropdownParent: $('.input-append #owner_list_wrapper')
                });

            }


            function getOwnerCount(methodName) {
                var methodNameUpper = methodName.toUpperCase();
                if (methodNameUpper == 'SELECTEDOWNERS' || methodNameUpper == 'ALLOWNERS' || methodNameUpper == 'TOPDEPOSITEDOWNERS' ||
                    methodNameUpper == 'NOTLOGINOWNERS') {
                    $('.ownerCount').text(0);
                    $('.ownerCountText').addClass('d-none');
                    return;
                }
                var route = "{{ route('admin.owners.segment.count', ':methodName') }}"
                route = route.replace(':methodName', methodName)
                $.get(route, function(response) {
                    $('.ownerCount').text(response);
                    $('.ownerCountText').removeClass('d-none');
                });
            }
